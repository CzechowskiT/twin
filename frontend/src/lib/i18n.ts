...
const en = {
  ...
  investorCalc: {
    ...
    liveStatsOn: "Ready",
    liveStatsOff: "Off",
    liveStatsPulseEyebrow: "Weekly product pulse",
    liveStatsApplied7d: "Marked applied (7d)",
    liveStatsCvUpload30d: "CV uploads (30d)",
    ...
    beBannerEyebrow: "Projection highlight",
  ...
  demo: {
    ...
    pageLead:
      "Below is a synthetic candidate and a fictional listing shaped like production data. The match score uses the same rule-based engine as the dashboard. The checklist animates locally; when it finishes, this page calls the public GET /api/v1/public/demo-receipt (no auth) for a fixed sandbox receipt and matching .ics download — no employer portal is contacted here. Real Playwright auto-apply runs only after sign-in, consent, and a paid plan.",
    simulationTitle: "Sandbox story + live public API",
    simulationBody:
      "The staged steps are for UX only. The closing receipt comes from our API in sandbox mode (no database application row, no calendar write server-side). Production auto-apply uses POST /api/v1/applications/auto-apply in your authenticated session.",
    ...
    stepCalendar: "Propose screening slot & calendar file",
    stepPending: "Waiting",
    stepActive: "In progress…",
    stepDone: "Complete",
    receiptTitle: "Orchestration receipt (sandbox)",
    receiptRefLabel: "Reference",
    receiptApplyLabel: "Apply (illustrative)",
    receiptJobLabel: "Role",
    receiptInterviewLabel: "Screening window (UTC)",
    receiptLoading: "Fetching sandbox receipt…",
    receiptError: "Could not load the public demo receipt. Check that the API URL is configured for this deployment.",
    downloadIcsCta: "Download .ics",
    footerNote:
...
const pl: MessageTree = {
  ...
  investorCalc: {
    ...
    liveStatsPulseEyebrow: "Puls tygodniowy produktu",
    liveStatsApplied7d: "Status „applied” (7 dni)",
    liveStatsCvUpload30d: "Wgrania CV (30 dni)",
    ...
    beBannerEyebrow: "Wyróżnik projekcji",
  ...
  demo: {
    ...
    pageLead:
      "Poniżej syntetyczny kandydat i fikcyjna oferta w kształcie danych produkcyjnych. Wynik dopasowania liczy ten sam silnik reguł co w panelu. Lista kroków animuje się lokalnie; po zakończeniu strona woła publiczne GET /api/v1/public/demo-receipt (bez logowania) po stały paragon sandbox i plik .ics — żaden portal pracodawcy nie jest tu wywoływany. Prawdziwy auto-apply z Playwright działa dopiero po zalogowaniu, zgodach i płatnym planie.",
    simulationTitle: "Sandbox + żywe publiczne API",
    simulationBody:
      "Kroki są pod UX; końcowy paragon pochodzi z naszego API w trybie sandbox (bez wiersza aplikacji w bazie, bez zapisu kalendarza po stronie serwera). W produkcji auto-apply używa POST /api/v1/applications/auto-apply w uwierzytelnionej sesji.",
    ...
    stepCalendar: "Slot screeningowy i plik kalendarza",
    stepPending: "Oczekuje",
    stepActive: "W toku…",
    stepDone: "Gotowe",
    receiptTitle: "Paragon orchestracji (sandbox)",
    receiptRefLabel: "Numer referencyjny",
    receiptApplyLabel: "Aplikacja (ilustracja)",
    receiptJobLabel: "Rola",
    receiptInterviewLabel: "Okno screeningu (UTC)",
    receiptLoading: "Pobieranie paragonu sandbox…",
    receiptError: "Nie udało się wczytać publicznego paragonu demo. Sprawdź konfigurację adresu API dla tego wdrożenia.",
    downloadIcsCta: "Pobierz .ics",
    footerNote:
