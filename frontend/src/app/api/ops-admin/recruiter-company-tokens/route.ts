...
function serverOpsToken(): string | undefined {
  return (process.env.OPS_ADMIN_TOKEN || process.env.BETA_ADMIN_TOKEN)?.trim();
}

export async function GET(req: Request) {
  const serverToken = serverOpsToken();
  if (!serverToken) {
  ...
  const url = `${base.replace(/\/$/, "")}/api/v1/admin/recruiter-company-tokens`;
