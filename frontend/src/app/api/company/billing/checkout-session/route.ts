...
/** Sandbox-only company billing checkout — proxies backend; never claims public Stripe launch. */
export async function POST(req: Request) {
  const gate = recruiterInboxProxyGate(req);
  if (gate) return gate;
  const base = getUpstreamApiBase();
  const url = new URL(req.url);
  const upstreamUrl = `${base!.replace(/\/$/, "")}/api/v1/company/billing/checkout-session?${url.searchParams.toString()}`;
