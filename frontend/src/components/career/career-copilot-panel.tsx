...
/**
 * Career Copilot 2.0 panel — persistent advisor on /dashboard/career.
 * Claim kinds: FACT | INFERENCE | SUGGESTION | UNKNOWN. No Wave/Gate jargon.
 */
...
type Claim = { kind?: string; text?: string; confidence?: string };
type Direction = {
  path_key: string;
  title: string;
  status: string;
...
type Aggregate = {
  ...
  decisions?: { id: number; title: string; comparison?: { options?: Record<string, { risk?: string; salary_trend?: string; pros?: string[]; cons?: string[] }> } }[];
  explainability?: { claim_kinds?: string[] };
  safety?: { ai_kill_switch?: boolean };
};

const TABS = ["overview", "directions", "roadmap", "goals", "decisions"] as const;
...
export function CareerCopilotPanel() {
  ...
  async function overrideDirection(pathKey: string, action: "accept" | "reject" | "restart") {
    ...
    setBusy(true);
    try {
      await apiFetch(`${API}/directions/${encodeURIComponent(pathKey)}/override`, {
        method: "POST",
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2" data-testid="career-copilot-overview">
          <div className="rounded border border-[var(--twin-border)] p-3">
            ...
            <p className="mt-1 text-sm font-medium">
              {ov?.where_am_i?.role || t("careerCopilot.roleUnknown")} ·{" "}
              {ov?.where_am_i?.skills_count ?? 0} {t("careerCopilot.skills")}
            </p>
          ...
          <div className="rounded border border-[var(--twin-border)] p-3 sm:col-span-2">
            ...
            <p className="mt-1 text-sm">
              {t("careerCopilot.salaryBand")}: {data?.market?.salary_band || "UNKNOWN"} ·{" "}
              {t("careerCopilot.demand")}: {data?.market?.role_demand || "UNKNOWN"}
            </p>
              ...
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  ...
                  <p className="twin-muted mt-1 text-xs">
                    ...
                    {t("careerCopilot.market")}: {d.market_demand} · {t("careerCopilot.salaryBand")}:{" "}
                    {d.salary_trend}
                  </p>
                  <ClaimBadge kind="SUGGESTION" />
                </div>
              ...
              <div className="mt-2 grid gap-2 sm:grid-cols-2">
                {Object.entries(d.comparison?.options || {}).map(([key, val]) => (
                  <div key={key} className="rounded bg-[var(--twin-border)]/20 p-2 text-xs">
                    ...
                    <p>
                      {t("careerCopilot.risk")}: {val.risk} · {t("careerCopilot.salaryBand")}:{" "}
                      {val.salary_trend}
                    </p>
                    <ClaimBadge kind="UNKNOWN" />
                  </div>
