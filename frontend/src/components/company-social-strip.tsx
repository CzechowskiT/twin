    ...
    <div className="rounded-2xl border border-[var(--twin-border)] bg-[var(--twin-card)]/60 px-5 py-5 sm:px-6">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-center gap-3 sm:justify-start">
        {networks.map(({ href, Icon, aria }) =>
          href ? (
            <a key={aria} href={href} target="_blank" rel="noopener noreferrer" className={linkClass} aria-label={t(aria)}>
              <Icon className="h-5 w-5" />
            </a>
          ) : (
            <span key={aria} className={`${linkClass} cursor-not-allowed opacity-45`} title={soon} aria-label={t(aria)}>
              <Icon className="h-5 w-5" />
            </span>
          ),
        )}
