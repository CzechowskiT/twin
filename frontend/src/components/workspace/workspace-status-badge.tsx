...
export function WorkspaceStatusBadge({ status }: { status: WorkspaceModuleStatus }) {
  const { t } = useTranslation();
  return (
    <span
      className={`inline-flex shrink-0 rounded-full border px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${TONE[status]}`}
