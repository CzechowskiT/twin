/**
 * Interactive demo a11y — axe-core on homepage story + /demo player.
 *
 * Local:
...
test.describe("Interactive demo a11y", () => {
  ...
  test("/demo interactive player has no serious axe violations", async ({ browser }) => {
    await withFreshContext(browser, async (context) => {
      const page = await context.newPage();
      await page.goto("/demo", { waitUntil: "domcontentloaded" });
      await dismissCookieBanner(page);
      await expect(page.locator("[data-interactive-demo-player]")).toBeVisible({ timeout: 15_000 });

      const results = await new AxeBuilder({ page })
        .include("[data-interactive-demo-player]")
        .withTags(["wcag2a", "wcag2aa", "wcag21a", "wcag21aa"])
        .analyze();
