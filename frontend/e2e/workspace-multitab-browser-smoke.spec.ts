...
async function freshContext(browser: { newContext(): Promise<BrowserContext> }) {
...
async function closeAll(pages: Page[], context: BrowserContext) {
...
test.describe("workspace multi-tab browser smoke", () => {
  ...
  test("7 module link cmd-click opens new tab at href", async ({ browser }) => {
    ...
    await closeAll([page, popup], context);
  });
