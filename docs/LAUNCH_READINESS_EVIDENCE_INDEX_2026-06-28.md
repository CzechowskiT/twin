...
| **Gate E Phase 3B split-batch execution** | **SHIPPED, STATICALLY VERIFIED — NOT DISPATCHED** — [split-batch execution plan](./GATE_E_PHASE3B_SPLIT_BATCH_EXECUTION_PLAN_2026-07-03.md) — splits the single long-lived `gate-e-phase3b-prod` job (the shape behind attempt 12's `RUNNER_CANCELLED` and attempt 13's `RUNNER_LOST_COMMUNICATION`) into 3 sequential matrix jobs (`public-candidate`, `recruiter`, `company`; `fail-fast: false`, `max-parallel: 1`), each uploading its own evidence artifact `if: always()`, plus a best-effort `gate-e-phase3b-aggregate` job that merges per-batch diagnostics without drawing a new pass/fail verdict; verified via `tsc`, extended `test:gate-e-isolated-runner-guard`, new `test:gate-e-phase3b-split-batch`, and `npm run build`; **attempt 14 still requires a separate, explicit founder authorization, NOT authorized by this document** |
| **Gate F** | **PENDING** — production smoke boundaries / re-audit |
| **Phase 3B** | **FAIL** — prod multitab executed; blank-or-no-content 20/20; harness hardened (PR #353) — **not fixed by runtime evidence**; isolated-runner attempt 12 dispatch 2 (`28652257796`) passed all preconditions then was `RUNNER_CANCELLED` before route evidence; attempt 13 (`28661876288`) passed all preconditions, ran ~46m43s further into the canonical command, then was `RUNNER_LOST_COMMUNICATION` before confirmed route evidence — [attempt 12 result](./gate-e-phase3b-attempt12-result-2026-07-03.md) · [attempt 13 result](./gate-e-phase3b-attempt13-result-2026-07-03.md) |
