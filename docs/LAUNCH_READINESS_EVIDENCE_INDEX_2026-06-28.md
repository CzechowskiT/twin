...
| **Gate E Phase 3B route-level sharding** | **SHIPPED, STATIC-ONLY — NOT DISPATCHED** — [route-sharding plan](./GATE_E_PHASE3B_ROUTE_SHARDING_PLAN_2026-07-03.md) — shards `gate-e-phase3b-prod` one level finer than the split-batch shape: a 20-entry route matrix (one route per isolated-runner job, `fail-fast: false`, `max-parallel: 1`) instead of the 3-way batch matrix that attempt 14 exercised, so a single `RUNNER_SHUTDOWN_SIGNAL` (or any other infrastructure-level ending) can now cost at most 1 of 20 routes' evidence instead of an entire 6-7-route batch's; the `gate-e-phase3b-aggregate` job is rewritten to merge per-route (not per-batch) evidence into a 20-row summary; statically verified only, **not dispatched, does not authorize attempt 15** |
| **Gate F** | **PENDING** — production smoke boundaries / re-audit |
| **Phase 3B** | **FAIL** — prod multitab executed; blank-or-no-content 20/20; harness hardened (PR #353) — **not fixed by runtime evidence**; isolated-runner attempt 12 dispatch 2 (`28652257796`) passed all preconditions then was `RUNNER_CANCELLED` before route evidence; attempt 13 (`28661876288`) passed all preconditions, ran ~46m43s further into the canonical command, then was `RUNNER_LOST_COMMUNICATION` before confirmed route evidence; attempt 14 split-batch (`28771385932`) passed all preconditions and the preflight sub-test in all 3 independent batches, then every batch was hit by `RUNNER_SHUTDOWN_SIGNAL` before confirmed route evidence; route-level sharding (20-route matrix, statically verified, not yet dispatched) is the next mitigation, aimed at reducing how much evidence any one infrastructure-level ending can cost — [attempt 12 result](./gate-e-phase3b-attempt12-result-2026-07-03.md) · [attempt 13 result](./gate-e-phase3b-attempt13-result-2026-07-03.md) · [attempt 14 result](./gate-e-phase3b-attempt14-result-2026-07-03.md) · [route-sharding plan](./GATE_E_PHASE3B_ROUTE_SHARDING_PLAN_2026-07-03.md) |
...
15. **Gate E attempt 15 (route-level evidence, route-sharded isolated runner)** → **PENDING** — requires a separate, explicit founder **"Gate E attempt 15 (isolated runner, fifth dispatch, route-sharded) = YES?"** decision; no code fix is indicated as required by attempt 14's failure mode (harness/preconditions behaved correctly in all 3 batches before each was hit by the shutdown signal); the route-sharding change is a CI job-topology mitigation, not a claim that it will prevent a future infrastructure-level ending.
16. **Gate F / launch re-audit** → only after Gate D/E evidence and P0 performance review.
17. **Public launch GO** → separate founder decision; §5 launch gate matrix must be green; **not implied** by this index.
