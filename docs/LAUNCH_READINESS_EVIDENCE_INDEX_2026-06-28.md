...
| **Phase 3B** | **FAIL (8/20 PASS, 12/20 confirmed product failures)** — prod multitab executed via the route-sharded isolated runner (attempt 15, run `28777105356`); more specific evidence than the prior `BLANK_OR_NO_CONTENT` 0/20-confirmed FAIL (2026-06-28): 8 routes pass cleanly, 11 routes fail on a captured browser-side JS runtime error (`page-error:1`, concentrated on all 7 `/recruiter/*` routes plus 4 `/company/*` candidate-detail sub-routes), and 1 route (`/dashboard`) fails on excessive DOM node count (`dom-fail:21100`); isolated-runner attempts 12–14 (`28652257796`, `28661876288`, `28771385932`) each ended at the GitHub Actions infrastructure level (`RUNNER_CANCELLED`, `RUNNER_LOST_COMMUNICATION`, `RUNNER_SHUTDOWN_SIGNAL` respectively) before any confirmed route evidence; the route-level sharding mitigation shipped after attempt 14 appears to have prevented a recurrence on this dispatch — [attempt 12 result](./gate-e-phase3b-attempt12-result-2026-07-03.md) · [attempt 13 result](./gate-e-phase3b-attempt13-result-2026-07-03.md) · [attempt 14 result](./gate-e-phase3b-attempt14-result-2026-07-03.md) · [attempt 15 result](./gate-e-phase3b-attempt15-result-2026-07-03.md) |
