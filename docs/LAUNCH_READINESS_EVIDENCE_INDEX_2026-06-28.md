...
**Canonical status (2026-07-14):** **P0: CLOSED** | **Gate E: PASS (attempt 19, 20/20)** | **Gate F: PENDING** | **LB-104: CLOSED** (C1–C5 per-module evidence) | **LB-106: CLOSED** (soak #29315813862 PASS) | **Demo founder review: PASS 18/18** @ `934cbbeb` | **Launch: NO-GO** — [blocker register](./PUBLIC_LAUNCH_BLOCKER_REGISTER_2026-07-13.md) · [scorecard](./PUBLIC_LAUNCH_9_OF_10_SCORECARD.md) · [demo evidence](./DEMO_FOUNDER_REVIEW_EVIDENCE_2026-07-14.md).
...
| **Public launch** | **NO-GO** |
| **P0 performance** | **CLOSED** — [closure decision](./P0_CLOSURE_DECISION_2026-07-07.md) (founder RSS validation 2026-07-07); perf optimization → Performance 2.0 |
...
| **Gate E** | **YES / PASS** — Phase 3B prod **20/20 PASS** (attempt 19, run `28849996684`) — [attempt 19 result](./gate-e-phase3b-attempt19-result-2026-07-06.md); prior **0/20 FAIL** — [gate-e result](./gate-e-phase3b-result-2026-06-28.md) |
...
| **Gate E retry (attempt 11)** | **USER_ABORTED** — [attempt 11 result](./gate-e-phase3b-attempt11-result-2026-07-03.md) — ownership-scoped macOS detection (attempt 10 hardening) correctly reported `NEEDS_MANUAL_REVIEW` (38 unowned Chrome-family processes, `browserPid: null`) instead of a false all-clear; 0/20 routes evaluated; **INCONCLUSIVE**; local Phase 3B execution is now **hard-blocked** — [isolated runner plan](./GATE_E_ISOLATED_RUNNER_PLAN_2026-07-03.md) · [local execution disabled](./PHASE3B_LOCAL_EXECUTION_DISABLED_2026-07-03.md) |
| **Isolated Gate E Phase 3B runner** | **SHIPPED, LIVE-DISPATCHED TWICE** — `workflow_dispatch`-only GitHub Actions workflow ([gate-e-phase3b-manual.yml](../.github/workflows/gate-e-phase3b-manual.yml)) is now the **only** execution path for Gate E Phase 3B; every local invocation (prod or browser variant, npm script or direct `playwright test`) exits immediately with `"Local Phase 3B execution is disabled. Use the GitHub Actions workflow."` — [isolated runner plan](./GATE_E_ISOLATED_RUNNER_PLAN_2026-07-03.md) · [local execution disabled inventory](./PHASE3B_LOCAL_EXECUTION_DISABLED_2026-07-03.md) |
| **Gate E retry (attempt 12, isolated runner run `28650677999`)** | **PRECONDITION_FAILED (false positive, fixed)** — first isolated-runner dispatch; `public-health` 10/10 and 10-route HTTP smoke both passed read-only, then the resource watchdog refused to start reporting "10 orphaned playwright/phase3b processes," which were actually this run's own still-running npm/npx/playwright ancestor chain, not a leftover process; 0/20 routes evaluated; root-caused and fixed (ancestry-aware orphan exclusion on `GITHUB_ACTIONS=true`, local Mac behavior unchanged) — verified statically only, **not yet re-verified against a live rerun** — [resource watchdog addendum](./PHASE3B_RESOURCE_WATCHDOG_2026-07-03.md#6-addendum-2026-07-03--run-28650677999-false-positive-orphan-detection) · [isolated runner plan §5a](./GATE_E_ISOLATED_RUNNER_PLAN_2026-07-03.md#5a-run-28650677999-2026-07-03--first-isolated-runner-attempt--false-positive-orphan-detection) |
| **Gate E retry (attempt 12, dispatch 2, isolated runner run `28652257796`)** | **RUNNER_CANCELLED / INCONCLUSIVE** — [attempt 12 result](./gate-e-phase3b-attempt12-result-2026-07-03.md) — the dispatch-1 false positive did **not** recur (orphan check 0 before/after); `public-health` 10/10, HTTP smoke 10/10, and the Phase 3B "prod preflight" sub-test all **passed**; the GitHub Actions job was then **cancelled** at the infrastructure level before any of the 20-route batches executed — not a scripted precondition failure, not a shared-host resource-safety issue (ephemeral isolated runner), and not initiated by this task's own tooling (confirmed via API); 0/20 routes evaluated; **attempt 13 requires a separate, explicit founder authorization, NOT authorized by this document** |
| **Gate E isolated-runner cancellation hardening** | **SHIPPED, LIVE-EXERCISED (attempt 13)** — `[gate-e-heartbeat]` log lines at every checkpoint (before canonical, after preflight, before/after each batch, every 30s idle wait, final cleanup) + best-effort `frontend/.diagnostics/gate-e-attempt-status.json` persistence after every stage + explicit canonical-command step timeout (40min, under the 60min job timeout) — closed the "almost no forensic trail" gap attempt 12 exposed for a plain external cancel, but attempt 13 revealed a residual gap: heartbeats/status files are not retrievable after a `RUNNER_LOST_COMMUNICATION` ending because the job's log blob itself never finalizes; does not change Phase 3B pass/fail logic, does not weaken the resource watchdog — [attempt 13 result](./gate-e-phase3b-attempt13-result-2026-07-03.md) · [isolated runner plan §5b](./GATE_E_ISOLATED_RUNNER_PLAN_2026-07-03.md#5b-cancellation-hardening-this-task-2026-07-03--surviving-a-silent-github-actions-cancellation) |
| **Gate E retry (attempt 13, isolated runner run `28661876288`)** | **RUNNER_LOST_COMMUNICATION / INCONCLUSIVE** — [attempt 13 result](./gate-e-phase3b-attempt13-result-2026-07-03.md) — all preconditions passed (public-health 10/10, HTTP smoke 10/10, static guards, orphan check clean); canonical command ran **~46m43s** (far past attempt 12's ~4m21s) before the job ended; GitHub's own failure annotation reads *"The hosted runner lost communication with the server... starves it for CPU/Memory, or blocks its network access..."* — distinct from attempt 12's generic *"The operation was canceled."*; 0/20 routes confirmed; no artifacts, no retrievable job log; **attempt 14 requires a separate, explicit founder authorization, NOT authorized by this document** |
| **Gate E Phase 3B split-batch execution** | **SHIPPED, LIVE-DISPATCHED (attempt 14)** — [split-batch execution plan](./GATE_E_PHASE3B_SPLIT_BATCH_EXECUTION_PLAN_2026-07-03.md) — splits the single long-lived `gate-e-phase3b-prod` job (the shape behind attempt 12's `RUNNER_CANCELLED` and attempt 13's `RUNNER_LOST_COMMUNICATION`) into 3 sequential matrix jobs (`public-candidate`, `recruiter`, `company`; `fail-fast: false`, `max-parallel: 1`), each uploading its own evidence artifact `if: always()`, plus a best-effort `gate-e-phase3b-aggregate` job that merges per-batch diagnostics without drawing a new pass/fail verdict; first live dispatch (attempt 14, run `28771385932`) reached the batch-start/preflight-pass phase in all 3 independent runners before each was hit by a `RUNNER_SHUTDOWN_SIGNAL` — the isolation did not, on this dispatch, prevent GitHub Actions infrastructure-level non-completion — [attempt 14 result](./gate-e-phase3b-attempt14-result-2026-07-03.md) |
| **Gate E retry (attempt 14, split-batch, isolated runner run `28771385932`)** | **RUNNER_SHUTDOWN_SIGNAL / INCONCLUSIVE** — [attempt 14 result](./gate-e-phase3b-attempt14-result-2026-07-03.md) — first live dispatch of the split-batch workflow (PR #377, 3 independent matrix jobs); all preconditions passed in all 3 batches (public-health 10/10, HTTP smoke 10/10, static guards, orphan check clean) and even the canonical command's own "prod preflight" sub-test passed (confirmed via retrievable ✓ log line) in all 3; each batch's independently-provisioned runner then received an identical GitHub-provided annotation — *"The runner has received a shutdown signal... runner service is stopped, or a manually started runner is canceled"* + exit code 143 — a third, distinct infrastructure-failure signature from attempt 12's `RUNNER_CANCELLED` and attempt 13's `RUNNER_LOST_COMMUNICATION`; no consistent correlation between canonical-command duration (~2–25 minutes) and the failure; 0/20 routes confirmed across all 3 batches; only the aggregate artifact survives; **attempt 15 requires a separate, explicit founder authorization, NOT authorized by this document** |
| **Gate E Phase 3B route-level sharding** | **SHIPPED, LIVE-DISPATCHED (attempt 15)** — [route-sharding plan](./GATE_E_PHASE3B_ROUTE_SHARDING_PLAN_2026-07-03.md) — shards `gate-e-phase3b-prod` one level finer than the split-batch shape: a 20-entry route matrix (one route per isolated-runner job, `fail-fast: false`, `max-parallel: 1`) instead of the 3-way batch matrix that attempt 14 exercised; first live dispatch (attempt 15, run `28777105356`) completed **all 20 route jobs with zero infrastructure-level endings** — a first for this workflow — [attempt 15 result](./gate-e-phase3b-attempt15-result-2026-07-03.md) |
| **Gate E retry (attempt 15, route-sharded, isolated runner run `28777105356`)** | **FAIL (8/20 PASS, 12/20 confirmed product failures)** — [attempt 15 result](./gate-e-phase3b-attempt15-result-2026-07-03.md) — first live dispatch of the route-sharded workflow (PR #379, 20 independent route-level matrix jobs); all preconditions passed in all 20 routes (public-health 10/10, HTTP smoke 10/10, static guards, orphan check clean, preflight sub-test) and — unlike attempts 12–14 — every route job ran its canonical command to full natural completion with **zero** `RUNNER_CANCELLED`/`RUNNER_LOST_COMMUNICATION`/`RUNNER_SHUTDOWN_SIGNAL` endings; **8/20 routes PASS** cleanly, **12/20 routes FAIL** on genuine product-level harness assertions (11 via a captured JS `page-error:1`, all concentrated on `/recruiter/*` (7/7) and `/company/*` candidate-detail sub-routes; 1 via excessive DOM node count on `/dashboard`, `dom-fail:21100` vs. 15000 threshold); a separate `.diagnostics` artifact-upload config bug (`include-hidden-files` defaults to `false` on `actions/upload-artifact@v4`) meant the aggregate job's own summary shows all 20 routes as MISSING despite full route-level evidence being reconstructible from job logs + `playwright-report`/`test-results`; **attempt 16 requires a separate, explicit founder authorization, NOT authorized by this document** |
...
| **E** | Phase 3B unblock? | **YES / PASS** | [attempt 19 result](./gate-e-phase3b-attempt19-result-2026-07-06.md) — **20/20 PASS** prod multitab (canonical); prior [gate-e result](./gate-e-phase3b-result-2026-06-28.md) **0/20 FAIL** |
| **F** | Prod smoke boundaries / re-audit? | **PENDING** | Re-audit executed — [GATE_F_REAUDIT_RESULT_2026-07-07.md](./GATE_F_REAUDIT_RESULT_2026-07-07.md); founder decision not recorded |
...
| **Gate E retry (attempt 11)** | **USER_ABORTED / INCONCLUSIVE** — [attempt 11 result](./gate-e-phase3b-attempt11-result-2026-07-03.md); ownership-scoped detection reported `NEEDS_MANUAL_REVIEW` (38 unowned Chrome-family processes), 0/20 routes evaluated; **local Phase 3B execution is now hard-blocked** — no further local attempts are possible; see [isolated runner plan](./GATE_E_ISOLATED_RUNNER_PLAN_2026-07-03.md) |
| **Gate E retry (attempt 12, dispatch 2, isolated runner)** | **RUNNER_CANCELLED / INCONCLUSIVE** — [attempt 12 result](./gate-e-phase3b-attempt12-result-2026-07-03.md); run `28652257796` passed every precondition (public-health 10/10, HTTP smoke 10/10, Phase 3B prod-preflight sub-test) and the dispatch-1 false-positive orphan detection did **not** recur, but the GitHub Actions job was cancelled at the infrastructure level before any of the 20 routes were evaluated; not attributable to a scripted precondition, shared-host resource issue, or this task's tooling |
| **Gate E retry (attempt 13, isolated runner)** | **RUNNER_LOST_COMMUNICATION / INCONCLUSIVE** — [attempt 13 result](./gate-e-phase3b-attempt13-result-2026-07-03.md); run `28661876288` passed every precondition, ran ~46m43s into the canonical command (far past attempt 12's ~4m21s), then the job ended with GitHub's own annotation attributing it to the runner losing communication with the Actions service (CPU/Memory/network named as candidate causes) — a distinct signature from attempt 12's generic cancel text; 0/20 routes confirmed; no artifacts, no retrievable job log; **attempt 14 requires a separate founder authorization** |
...
11. ~~**Gate E attempt 11 (route-level evidence)**~~ → **DONE (USER_ABORTED)** — [attempt 11 result](./gate-e-phase3b-attempt11-result-2026-07-03.md) — ownership-scoped detection correctly reported `NEEDS_MANUAL_REVIEW`; 0/20 routes evaluated; local execution now hard-blocked.
12. ~~**Gate E attempt 12 (route-level evidence, isolated runner)**~~ → **DONE (dispatch 1: PRECONDITION_FAILED/false-positive, fixed; dispatch 2: RUNNER_CANCELLED)** — [attempt 12 result](./gate-e-phase3b-attempt12-result-2026-07-03.md) — dispatch 2 (run `28652257796`) passed every precondition, then the job was cancelled at the infrastructure level before any route evaluated; 0/20 routes evaluated.
13. ~~**Gate E attempt 13 (route-level evidence, isolated runner)**~~ → **DONE (RUNNER_LOST_COMMUNICATION)** — [attempt 13 result](./gate-e-phase3b-attempt13-result-2026-07-03.md) — run `28661876288` passed every precondition, ran ~46m43s into the canonical command, then the job ended with GitHub's own annotation attributing it to the runner losing communication with the Actions service — a distinct signature from attempt 12's plain cancellation; 0/20 routes confirmed.
14. ~~**Gate E attempt 14 (route-level evidence, split-batch isolated runner)**~~ → **DONE (RUNNER_SHUTDOWN_SIGNAL)** — [attempt 14 result](./gate-e-phase3b-attempt14-result-2026-07-03.md) — run `28771385932` (first live dispatch of the split-batch shape) passed every precondition and the preflight sub-test in all 3 independent batches, then every batch was hit by an identical GitHub-provided shutdown-signal annotation, exit 143; 0/20 routes confirmed. **Follow-up shipped (not yet dispatched):** route-level sharding (20-route matrix) — see [route-sharding plan](./GATE_E_PHASE3B_ROUTE_SHARDING_PLAN_2026-07-03.md) — aims to bound the evidence cost of any future infrastructure-level ending to at most 1 of 20 routes instead of an entire batch.
15. ~~**Gate E attempt 15 (route-level evidence, route-sharded isolated runner)**~~ → **DONE (FAIL, 8/20 PASS, 12/20 confirmed product failures)** — [attempt 15 result](./gate-e-phase3b-attempt15-result-2026-07-03.md) — run `28777105356` completed all 20 route jobs with **zero infrastructure-level endings** (a first for this workflow); 8/20 routes PASS, 12/20 FAIL on real product-level assertions (11 × `page-error:1`, 1 × `DOM_FAIL`); 20/20 routes confirmed.
16. **Gate E attempt 16 (route-level evidence, follow-up)** → **PENDING** — requires a separate, explicit founder **"Gate E attempt 16 = YES?"** decision; not urgently needed since attempt 15 already produced complete, actionable route-level evidence for all 20 routes; any further attempt would primarily verify whether the 12 confirmed product failures persist after an application-level fix (out of scope for a Gate E dispatch alone).
17. **Gate F / launch re-audit** → only after Gate D/E evidence and P0 performance review.
18. **Public launch GO** → separate founder decision; §5 launch gate matrix must be green; **not implied** by this index.
...
| [gate-e-phase3b-result-2026-06-28.md](./gate-e-phase3b-result-2026-06-28.md) | Gate E Phase 3B prod **0/20 FAIL** (historical reattempt) |
| [gate-e-phase3b-attempt19-result-2026-07-06.md](./gate-e-phase3b-attempt19-result-2026-07-06.md) | Gate E attempt 19 — **PASS 20/20** (canonical) |
| [gate-e-phase3b-attempt18-result-2026-07-06.md](./gate-e-phase3b-attempt18-result-2026-07-06.md) | Gate E attempt 18 — **superseded** by attempt 19 |
...
| [gate-e-phase3b-attempt11-result-2026-07-03.md](./gate-e-phase3b-attempt11-result-2026-07-03.md) | Gate E attempt 11 — **USER_ABORTED**, 0/20 routes evaluated; ownership-scoped detection reported `NEEDS_MANUAL_REVIEW` instead of a false all-clear |
| [GATE_E_ISOLATED_RUNNER_PLAN_2026-07-03.md](./GATE_E_ISOLATED_RUNNER_PLAN_2026-07-03.md) | Isolated `workflow_dispatch`-only GitHub Actions runner for Gate E Phase 3B — the founder Mac is retired as an execution host |
| [PHASE3B_LOCAL_EXECUTION_DISABLED_2026-07-03.md](./PHASE3B_LOCAL_EXECUTION_DISABLED_2026-07-03.md) | Every local Phase 3B execution path and how each is hard-blocked (`phase3b-prod-local-guard.ts`, spec-level guard) |
| [PHASE3B_RESOURCE_WATCHDOG_2026-07-03.md §6](./PHASE3B_RESOURCE_WATCHDOG_2026-07-03.md#6-addendum-2026-07-03--run-28650677999-false-positive-orphan-detection) | Run `28650677999` (first isolated-runner attempt) — **PRECONDITION_FAILED (false positive)** — watchdog's orphan-pattern check matched its own npm/npx/playwright ancestor chain; root-caused and fixed with `GITHUB_ACTIONS`-aware ancestry exclusion; local Mac behavior unchanged; 0/20 routes evaluated; fix verified statically only, **not yet re-verified live** |
| [gate-e-phase3b-attempt12-result-2026-07-03.md](./gate-e-phase3b-attempt12-result-2026-07-03.md) | Gate E attempt 12 — dispatch 1 (`28650677999`) PRECONDITION_FAILED/false-positive (fixed); dispatch 2 (`28652257796`) **RUNNER_CANCELLED** — orphan-detection fix confirmed working live (0 false positives), public-health 10/10, HTTP smoke 10/10, and Phase 3B prod-preflight sub-test all passed, then the job was cancelled at the GitHub Actions infrastructure level before any route batch executed; 0/20 routes evaluated |
| [gate-e-phase3b-attempt13-result-2026-07-03.md](./gate-e-phase3b-attempt13-result-2026-07-03.md) | Gate E attempt 13 — run `28661876288` **RUNNER_LOST_COMMUNICATION** — all preconditions passed; canonical command ran ~46m43s (far past attempt 12's ~4m21s) then the job ended with GitHub's own annotation attributing it to the runner losing communication with the Actions service (CPU/Memory/network), distinct from attempt 12's generic cancel text; 0/20 routes confirmed; no artifacts, no retrievable job log; attempt 14 requires separate founder authorization |
| [GATE_E_PHASE3B_SPLIT_BATCH_EXECUTION_PLAN_2026-07-03.md](./GATE_E_PHASE3B_SPLIT_BATCH_EXECUTION_PLAN_2026-07-03.md) | Splits the single long-lived Gate E Phase 3B prod job into 3 sequential isolated-runner matrix jobs (one per route batch) plus a best-effort aggregation job — statically verified, not dispatched, does not authorize attempt 14; **superseded by route-level sharding** (below) after attempt 14's `RUNNER_SHUTDOWN_SIGNAL` struck all 3 batches |
| [GATE_E_PHASE3B_ROUTE_SHARDING_PLAN_2026-07-03.md](./GATE_E_PHASE3B_ROUTE_SHARDING_PLAN_2026-07-03.md) | Shards the Gate E Phase 3B prod job one level finer than the split-batch shape: a 20-entry route matrix (one route per isolated-runner job) plus a rewritten per-route aggregation job — first live-dispatched in attempt 15 |
| [gate-e-phase3b-attempt15-result-2026-07-03.md](./gate-e-phase3b-attempt15-result-2026-07-03.md) | Gate E attempt 15 — first live dispatch of the route-sharded workflow, run `28777105356`; **FAIL (8/20 PASS, 12/20 confirmed product failures)** — zero infrastructure-level endings across all 20 routes (a first for this workflow); 11 routes fail on a captured JS `page-error:1`, 1 route (`/dashboard`) fails on excessive DOM node count; documents a separate `.diagnostics` artifact-upload config bug (`include-hidden-files` default) that left the aggregate job's summary showing all routes as MISSING despite full route-level evidence being reconstructible from job logs |
...
- Gate D prod browser executed once (canonical command); Phase 3B **PASS 20/20** (attempt 19); no default CI browser.
- No backend/API/auth/DB/env/smoke.yml changes.
- No launch GO, no Gate F YES claimed.

**Public launch: NO-GO · P0: CLOSED · Gate D: YES/PASS · Gate E: YES/PASS (20/20 attempt 19, run 28849996684) · Gate F: PENDING (re-audit executed) · Phase 3B: PASS (20/20 attempt 19) · Performance 2.0 backlog (not P0 blockers)**

Historical timeline (attempts 1–18) preserved in source documents below; attempt 18 superseded by attempt 19. Prior footer stance through attempt 15: **Public launch: NO-GO · P0: OPEN · Gate D: YES/PASS · Gate E: YES/FAIL · Phase 3B: FAIL (8/20 PASS, 12/20 confirmed product failures — attempt 15, run 28777105356) · Gate E post-harness retry: PARTIAL/AUTH_TOKEN_REQUIRED · Gate E with-token retry (attempts 3+4): PARTIAL/AUTH_TOKEN_REQUIRED · Gate E with-token retry (attempt 5): PARTIAL/HARNESS_LOAD_FAILURE (browser executed once, 0/20 routes evaluated, loader fix unverified by browser) · Gate E with-token retry (attempt 6): ABORTED_RESOURCE_SAFETY/INCONCLUSIVE (command started, manually interrupted for local resource safety, 0/20 routes evaluated, no automatic retry, attempt 7 NOT authorized) · Gate E attempt 7: PRECONDITION_FAILED/INCONCLUSIVE (prod public-health degraded, 0/20 routes evaluated) · Gate E attempt 8: PRECONDITION_FAILED/INCONCLUSIVE (host on battery power, 0/20 routes evaluated) · Gate E attempt 9: ABORTED_RESOURCE_SAFETY/MANUAL_ABORT/INCONCLUSIVE (AC power passed, manually aborted before Playwright for resource safety, 0/20 routes evaluated) · Gate E attempt 10: USER_ABORTED/INCONCLUSIVE (resource watchdog reported chrome-headless-shell=0, operator observed real Chrome/Chromium process pressure the single-process-name detection couldn't see, 0/20 routes evaluated) · Gate E attempt 11: USER_ABORTED/INCONCLUSIVE (ownership-scoped detection reported NEEDS_MANUAL_REVIEW, 0/20 routes evaluated, local execution now hard-blocked) · Gate E attempt 12: dispatch 1 PRECONDITION_FAILED/false-positive (fixed), dispatch 2 RUNNER_CANCELLED/INCONCLUSIVE (all preconditions passed, job cancelled before route evidence, 0/20 routes evaluated) · Gate E attempt 13: RUNNER_LOST_COMMUNICATION/INCONCLUSIVE (all preconditions passed, canonical command ran ~46m43s then runner lost communication with GitHub Actions per GitHub's own annotation, 0/20 routes confirmed, no artifacts, no retrievable log) · Gate E attempt 14: RUNNER_SHUTDOWN_SIGNAL/INCONCLUSIVE (all 3 split-batch runners passed every precondition then each hit an identical GitHub shutdown-signal annotation, 0/20 routes confirmed) · Gate E attempt 15 (route-sharded): FAIL (all 20 routes ran to completion with ZERO infrastructure-level endings — a first for this workflow; 8/20 PASS, 12/20 confirmed product failures — 11× page-error:1 concentrated on /recruiter/* and /company/* candidate-detail sub-routes, 1× DOM_FAIL on /dashboard) · Attempt 16: requires separate founder authorization, NOT AUTHORIZED, NOT RUN**
