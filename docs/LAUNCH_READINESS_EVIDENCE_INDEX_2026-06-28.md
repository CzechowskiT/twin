...
| **Isolated Gate E Phase 3B runner** | **SHIPPED** — `workflow_dispatch`-only GitHub Actions workflow ([gate-e-phase3b-manual.yml](../.github/workflows/gate-e-phase3b-manual.yml)) is now the **only** execution path for Gate E Phase 3B; every local invocation (prod or browser variant, npm script or direct `playwright test`) exits immediately with `"Local Phase 3B execution is disabled. Use the GitHub Actions workflow."` — [isolated runner plan](./GATE_E_ISOLATED_RUNNER_PLAN_2026-07-03.md) · [local execution disabled inventory](./PHASE3B_LOCAL_EXECUTION_DISABLED_2026-07-03.md) |
