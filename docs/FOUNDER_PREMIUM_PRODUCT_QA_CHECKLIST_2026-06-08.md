...
1. Run in **PL** and **EN** on production (or latest Vercel preview after merge). Spot-check **DE** on `/login` and `/register` (role cards must not show English “Candidate”, “Companies”, “Open workspace”). Spot-check **ES** on `/demo`, `/dashboard`, `/recruiter/inbox` for non-English premium copy (see `docs/I18N_FULL_TRANSLATION_COVERAGE_2026-06-08.md`).
