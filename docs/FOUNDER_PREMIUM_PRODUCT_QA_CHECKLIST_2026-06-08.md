...
| 1.4c | `/login/candidate` login options | Google/GitHub/Microsoft **visible immediately** (checking → clickable when `public-health` confirms); **Sprawdzamy dostępność logowania…** helper only — not blank section; expand **Pokaż wszystkie opcje logowania**; no layout pop-in (`docs/MULTI_TAB_PERFORMANCE_HARDENING_2026-06-16.md` · `test:login-options-instant-render`) |
...
| Multi-tab (8–12 tabs) | Workspace tabs: static 6-logo strip; background tabs: marquee paused, CPU/memory drop; split chrome headers (no marketing arrays on workspace); module cards use real `Link` hrefs (cmd-click deep link); `test:p0-renderer-memory-bundle-reduction` + `test:p0-browser-memory-multitab-performance` + `test:multi-tab-performance-hardening` + `test:workspace-deeplink-new-tab` + `test:workspace-multitab-browser-smoke` green — see `docs/P0_WORKSPACE_DEEPLINK_MULTITAB_2026-06-16.md` |
