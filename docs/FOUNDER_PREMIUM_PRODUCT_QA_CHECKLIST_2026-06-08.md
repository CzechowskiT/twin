...
| 1.4b | `/login/candidate` email login | Submit returns **error or redirect within 10s** — never infinite **Logowanie…**; invalid creds → **Nieprawidłowy e-mail lub hasło.**; upstream down → **Nie udało się zalogować…** (`docs/CANDIDATE_LOGIN_STUCK_INCIDENT_2026-06-11.md`) |
| 1.4c | `/login/candidate` login options | Google/GitHub/Microsoft rows **visible** (enabled or greyed); **Pokaż wszystkie opcje logowania** expands when collapsed; LinkedIn not stuck on **…** >10s (`docs/AUTH_LOGIN_AND_OPTIONS_INCIDENT_2026-06-11.md`) |
