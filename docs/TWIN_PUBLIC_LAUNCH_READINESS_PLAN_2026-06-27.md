...
| 2026-07-02 | **Slice 43 (frontend-only)** — `/api/public-health` proxy stability fix: health + celery-status now fetched **in parallel** (`Promise.allSettled`), fixing the proven root cause of intermittent prod 502s (sequential fetches + ~4.3-4.5s celery introspection pushing total latency past the 10s platform timeout) and an unguarded `celeryRes.json()` that produced empty 500s on bad bodies; celery-status is now **soft-fail-only** (`celery: {}` + `celery_warning`, never crashes the route); `db_ok`/`status` remain health-authoritative (Gate E compat preserved); optional `?mode=liveness` fast path added; `test:public-health-route-stability` (9 tests, mocked fetch, no network/browser); **no backend/API/DB/env change**; prod improvement requires a Vercel deploy; Launch **NO-GO**; P0 **OPEN**; Gate F **PENDING** |
