...
| Red Console direct `GET https://icons.duckduckgo.com/ip3/*.ico` or `https://t*.gstatic.com/faviconV2` → **404** (no CSP line) | **No** — non-CSP favicon CDN miss; **fix shipped** — blocklisted domains initials-only; remaining brands SI/`extraUrls` only (no Google/gstatic/DDG raster hops) — **post-deploy smoke on `/`** |
