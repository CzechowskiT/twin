...
| Chrome too many initials-only plates (founder post-`6d08742`) | **Found** — phantom `MARQUEE_STABLE_SI_SLUGS` (404 SI hops); **trimmed** in `dc4e906` |
| Chrome mostly initials after `dc4e906` (founder) | **Found** — domain/slug lookup + white-plate contrast + finance-heavy early order; **`MARQUEE_BRAND_LOGO_MAP`** + `/000000` SI fallback + logos-first sort; **post-deploy visual smoke required** |
| Enforce header | **Absent** (unchanged) |
| Decision | **CONTINUE** 72h burn-in — report-only HOLD |
| S2 status | **NOT READY** |
| S2 PASS | **NO** |
