...
3. Use **`action=start`** whenever the founder asks for code, tests, commits, PR, merge, deploy, policy change, or regression. Use **`action=analyze`** only for explicit read-only diagnosis (“przeanalizuj / bez zmian / without mutations”). Never silently downgrade mutating work to analyze.
...
2. `createTwinCommand` with a clear `direction`. Prefer `action=start` for real work; `action=analyze` only for read-only.
