...
| 2026-06-11 | post-audit closure owner | — (drill not executed) | staging clone `staging-restore-proof-20260529` (expected) | **NOT DONE / BLOCKED** | — | no founder Railway session in agent closure | Read-only prod baseline: `public-health` + Railway health `git_commit=e48bff1`, `db_ok=true` (scaffold HEAD `adfcac0` — 1 commit ahead); Alembic scaffold head `057_candidate_evidence_items` | **NO-GO** | Post-audit re-drill **required** after prod redeploy to scaffold HEAD; prior 2026-06-01 PASS **not revoked**; founder steps: `docs/O7_RESTORE_DRILL_RUNBOOK_2026-06-11.md`; **no fake PASS** |
| 2026-07-15 | autonomous agent (Railway CLI) | Read-only **pg_dump** prod Postgres → `twin_o7_prod_20260715T143323Z.dump` (294K, sha256 `308d5121…1c73`) | **staging clone** env `staging-restore-proof-20260529` / `Postgres-HE2P` / `postgres-volume-p1D7` (host ≠ prod) | **PASS** | restore **93 s**; forward migrate staging **46 s** | none on staging path | SQL: restore head `050_stripe_webhook_events` counts users **3** / candidates **3** / job_matches **1091** / applications **17**; staging then `alembic upgrade head` → `077_candidate_activity_timeline`; prod left at **050**/43 tables; full write-up `docs/O7_RESTORE_DRILL_EVIDENCE_2026-07-15.md` | **GO (O7)** · Launch **NO-GO** | Prior interrupted restore damaged prod public volume → recovered from same dump (exit 0, 154 s) before staging drill; **no** secrets/dump in git; residual: prod schema lag vs API (R-019 blocked) |
| 2026-07-23 | autonomous agent (Railway CLI, fingerprint-guarded) | Read-only **pg_dump** prod Postgres → `twin_o7_prod_20260723T065951Z.dump` (1.1 MB, sha256 `f3f239b0…e7ab`) | **staging clone** env `staging-restore-proof-20260529` / `Postgres-HE2P` / `postgres-volume-p1D7` (host `monorail…:39789` ≠ prod `autorack…:40861`) | **PASS** | restore **396 s** | none | SQL: alembic `096_connector_secret_hash_widen`; users **23** / candidates **17** / job_matches **1431** / applications **20** / tables **116** — staging matches prod; prod unchanged post-drill; write-up `docs/O7_RESTORE_DRILL_EVIDENCE_2026-07-23.md` | **GO (O7)** · Launch **NO-GO** · Gate F **PENDING** · Pilot **BLOCKED_BY_FOUNDER** | Fresh post-`096` re-drill; no prod volume destroy; no migration downgrade; dump already at head (no staging forward migrate) |
