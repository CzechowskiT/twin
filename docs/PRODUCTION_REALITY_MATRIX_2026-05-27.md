...
- **Candidate offers/matches real content (2026-06-16):** distinct `/dashboard/jobs` + `/dashboard/matches` — no panel bounce · `test:candidate-offers-matches-real-content` · launch **NO-GO** unchanged
- **Founder-led demo flow (2026-06-16):** `docs/FOUNDER_LED_DEMO_FLOW_2026-06-16.md` — `/demo` product walkthrough, role/journey cards, trust boundaries; no shell/gate changes; launch **NO-GO** unchanged
- **Candidate Profile 360 (2026-06-16):** `docs/CANDIDATE_PROFILE_360_2026-06-16.md` — `/recruiter/candidates/[id]` + company alias; pilot `demo-candidate-001`; `test:candidate-profile-360`; launch **NO-GO** unchanged
- **Job-Specific Pipeline (2026-06-17):** `docs/JOB_SPECIFIC_PIPELINE_STATUSES_2026-06-17.md` — `/recruiter/jobs/[jobId]/pipeline` + company alias; pilot `demo-role-001`; 7 stages; `test:job-specific-pipeline-statuses`; launch **NO-GO** unchanged
- **Notes, Feedback, Scorecards & Forms (2026-06-17):** `docs/NOTES_FEEDBACK_SCORECARDS_FORMS_2026-06-17.md` — collaboration workspace + job feedback/scorecards; pilot `demo-candidate-001` / `demo-role-001`; `test:notes-feedback-scorecards`; launch **NO-GO** unchanged
- **GDPR / Consent / Contact History (2026-06-17):** `docs/GDPR_CONSENT_CONTACT_HISTORY_2026-06-17.md` — trust workspace + consent/contact-history aliases; pilot `demo-candidate-001` / `demo-role-001`; `test:gdpr-consent-contact-history`; launch **NO-GO** unchanged
- **Team Collaboration Layer (2026-06-17):** `docs/TEAM_COLLABORATION_LAYER_2026-06-17.md` — team workspace + tasks alias; pilot `demo-candidate-001` / `demo-role-001`; 8 sections; `test:team-collaboration-layer`; launch **NO-GO** unchanged
- **Safe Email Communication Layer (2026-06-17):** `docs/SAFE_EMAIL_COMMUNICATION_LAYER_2026-06-17.md` — communication workspace + drafts alias; pilot `demo-candidate-001` / `demo-role-001`; draft-only; `test:safe-email-communication-layer`; launch **NO-GO** unchanged
- **ATS Import / Connector Readiness (2026-06-17):** `docs/ATS_IMPORT_CONNECTOR_READINESS_2026-06-17.md` — import readiness workspace + mapping/dedupe views; pilot connector matrix; mapping/review only; `test:ats-import-connector-readiness`; launch **NO-GO** unchanged
...
- **Performance-safe moving logo marquee (2026-06-16, logo quality + visual polish 2026-06-17):** `docs/PERFORMANCE_SAFE_MOVING_LOGO_MARQUEE_2026-06-16.md` — ≤27 DOM nodes (9×3 segments) on workspace/auth with inline curated wordmarks (`PerformanceSafeLogoMark`); marketing lazy-loads full strip; `test:performance-safe-moving-logo-marquee` + `test:performance-safe-moving-logo-marquee-browser`
...
| Recruiter compliance audit trail | `docs/RECRUITER_COMPLIANCE_AUDIT_TRAIL_MVP_2026-06-11.md` — append-only `recruiter_audit_events`; accept/decline + review-open; no decline notes in ledger; `test:recruiter-audit-trail-mvp` | **LIVE (pilot)** — inbox token scope; migration `053_recruiter_audit_events`; public launch **NO-GO** |
...
| Recruiter Talent Radar `/recruiter/talent-radar` | `docs/RECRUITER_TALENT_RADAR_MVP_2026-06-12.md` — `GET /api/v1/recruiter/talent-radar`; explainable resurfacing from internal context; decision cockpit UX (summary panel, fit bands, grouped cards); draft-only outreach; `test:recruiter-talent-radar-mvp` + `test:recruiter-talent-radar-premium-ux` | **PILOT** — Sourcing Memory Agent; **no** auto outreach / external sourcing; public launch **NO-GO** |
| Weekly Talent Radar Digest `/recruiter/talent-radar/digest` | `docs/RECRUITER_TALENT_RADAR_WEEKLY_DIGEST_2026-06-15.md` — `GET /api/v1/recruiter/talent-radar/digest`; deduplicated executive briefing + copy summary; `test:recruiter-talent-radar-weekly-digest` + `test:recruiter-talent-radar-weekly-digest-premium` | **PILOT** — **no** email send / auto outreach; public launch **NO-GO** |
| Recruiter Talent Pool Import `/recruiter/talent-pool` | `docs/RECRUITER_TALENT_POOL_IMPORT_MVP_2026-06-15.md` — CSV paste import; `GET /api/v1/recruiter/talent-pool`; preview/commit; Talent Radar `imported_internal_pool`; `test:recruiter-talent-pool-import-mvp` | **PILOT** — structured internal pool only; **no** live ATS sync / external sourcing; public launch **NO-GO** |
| Company Talent Pool View `/company/talent-pool` | `docs/COMPANY_TALENT_POOL_VIEW_MVP_2026-06-15.md` — `GET /api/v1/company/talent-pool`; executive summary + NBA + readiness guide + premium workspace selector; entry from `/for-companies`; `test:company-talent-pool-view-mvp` + `test:company-talent-pool-executive-ux` + `test:company-entry-navigation` | **PILOT** — organizational talent memory; **no** PII / auto outreach / live ATS; public launch **NO-GO** |
| Candidate Profile 360 `/recruiter/candidates/[id]` | `docs/CANDIDATE_PROFILE_360_2026-06-16.md` — recruiter/company system-of-record pilot; sample `demo-candidate-001`; 11 sections; meaningful not-found; `test:candidate-profile-360` | **PILOT** — demo data only; **no** live API / notes write / auto outreach; launch **NO-GO** |
| Job pipeline `/recruiter/jobs/[jobId]/pipeline` | `docs/JOB_SPECIFIC_PIPELINE_STATUSES_2026-06-17.md` — 7-stage board; sample `demo-role-001`; candidate cards + Profile 360 link; disabled stage actions; `test:job-specific-pipeline-statuses` | **PILOT** — demo data only; **no** stage mutation / ATS sync; launch **NO-GO** |
| Collaboration notes/feedback/scorecards | `docs/NOTES_FEEDBACK_SCORECARDS_FORMS_2026-06-17.md` — `/recruiter/candidates/[id]/collaboration` + job feedback/scorecards; 7 sections; 9-criteria scorecard; disabled note/form submit; `test:notes-feedback-scorecards` | **PILOT** — demo data only; **no** backend writes; launch **NO-GO** |
| Team collaboration `/recruiter/candidates/[id]/team` | `docs/TEAM_COLLABORATION_LAYER_2026-06-17.md` — shared workspace; activity timeline, assignments, tasks, checklist; job `/team` + `/tasks` aliases; `test:team-collaboration-layer` | **PILOT** — demo data only; **no** reassignment / task writes; launch **NO-GO** |
| Safe communication `/recruiter/candidates/[id]/communication` | `docs/SAFE_EMAIL_COMMUNICATION_LAYER_2026-06-17.md` — draft-only layer; consent warning, draft library, preview; job `/communication` + `/drafts` aliases; `test:safe-email-communication-layer` | **PILOT** — demo drafts only; **no** send / outreach / mailbox; launch **NO-GO** |
| ATS import readiness `/recruiter/integrations/ats/import-readiness` | `docs/ATS_IMPORT_CONNECTOR_READINESS_2026-06-17.md` — connector matrix, field mapping, dedupe, consent mapping, validation checklist; company alias; `test:ats-import-connector-readiness` | **PILOT** — mapping/review only; **no** live sync / writeback / credentials; launch **NO-GO** |
| GDPR / trust `/recruiter/candidates/[id]/trust` | `docs/GDPR_CONSENT_CONTACT_HISTORY_2026-06-17.md` — consent, contact permission, history timeline; job consent alias; `test:gdpr-consent-contact-history` | **PILOT** — demo data only; **no** contact send; launch **NO-GO** |
| Company marketing entry `/for-companies` | `docs/PERSONA_WORKSPACE_NAVIGATION_PREMIUM_UX_2026-06-12.md` — hero CTAs → `/company/dashboard`, `/company/talent-pool`; workspace preview; Panel header → dashboard; `test:company-entry-navigation` | **LIVE** — navigation only; procurement story unchanged; launch **NO-GO** |
| Recruiter manual scheduling | `docs/RECRUITER_SCHEDULING_MVP_MANUAL_2026-06-11.md` — inbox prepare-invite panel; manual slot + meeting link; copy invite; mark invited/scheduled; `test:recruiter-scheduling-mvp`; **no** outbound email, **no** calendar write | **REPO** — manual workflow only; recruiter calendar sync **NOT LIVE** |
| Recruiter pipeline `/recruiter/pipeline` | `docs/RECRUITER_SCHEDULING_MVP_MANUAL_2026-06-11.md` — ATS-lite stages; scheduled slot badge on cards; `test:recruiter-pipeline-mvp`; transitions via API | **REPO** — list + stage transitions; no external ATS sync |
...
| Candidate Google Calendar reconnect stability | `fix/candidate-google-calendar-reconnect-loop-2026-06-11` — access token cache, refresh-before-reconnect, OAuth refresh preserve, `temporary_error` + Retry; migration `052`; `test:candidate-google-calendar-stability` (`docs/CANDIDATE_GOOGLE_CALENDAR_RECONNECT_LOOP_FIX_2026-06-11.md`) | **LIVE** (candidate) — no reconnect loop on transient 429/503; Connect without mandatory disconnect |
| Candidate calendar loading state timeout | `fix/candidate-calendar-loading-state-timeout-2026-06-11` — 9s per-provider status/events timeout; terminal retry/connect UI; `test:candidate-calendar-loading-state` (`docs/CANDIDATE_CALENDAR_LOADING_STATE_TIMEOUT_FIX_2026-06-11.md`) | **LIVE** (candidate) — no indefinite **Ładowanie…**; Google hang does not block Microsoft |
| Candidate calendar P0 root-cause | `fix/p0-candidate-calendar-root-cause-2026-06-11` — provider phases no longer gated on untimed `auth/me`/ops health; `test:candidate-calendar-p0-root-cause` (`docs/CANDIDATE_CALENDAR_P0_ROOT_CAUSE_FIX_2026-06-11.md`) | **LIVE** — founder authenticated smoke recommended |
| Candidate calendar week events loading | `fix/p0-candidate-calendar-week-events-loading-2026-06-12` — `WeekEventsPhase` + stable fetch deps; `test:candidate-calendar-week-events-loading` (`docs/CANDIDATE_CALENDAR_WEEK_EVENTS_LOADING_FIX_2026-06-12.md`) | **LIVE** (deploy `51fa345`) — prod **PASS** only after founder confirms no hung **Ładowanie wydarzeń…** (`docs/FOUNDER_AUTHENTICATED_PERSONA_SMOKE_RUNBOOK_2026-06-12.md` §5) |
| Persona workspace auth gate UX | `fix/persona-audit-p0-p1-closure-2026-06-12` — `PersonaWorkspaceGate` sign-in required shell + role login redirect; `test:persona-workspace-gate-auth` | **LIVE** (FE) — no blank gated shell for unauthenticated users |
| Company integrations `/company/integrations` | `fix/persona-audit-p0-p1-closure-2026-06-12` — readiness stub (planned/not-live); `test:company-integrations-readiness-mvp` | **LIVE** (FE) — no fake connected claims |
| Premium i18n overlay coverage | `fix/persona-audit-p0-p1-closure-2026-06-12` — `test:i18n-coverage` regreen (848→0 EN fallbacks on premium paths) | **LIVE** (CI) |
