...
- **Auto-apply / delegated apply:** **PAUSED** / **NOT LIVE** — see `docs/AUTO_APPLY_DELEGATED_APPLY_SAFETY_AUDIT_2026-06-02.md`; `delegated_apply_allowed=false`; ops `trigger-sweep` allowlist-only; UI omits trigger-sweep; **HIGH** follow-up: align `POST /applications/auto-apply` with `autonomous_apply_allowed()` before public launch widen.
