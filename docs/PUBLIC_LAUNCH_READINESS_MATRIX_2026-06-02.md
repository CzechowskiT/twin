...
- **Marketing logo marquee:** Pre-fix `/_next/image` noise classified **non-CSP**; branch fix uses direct `<img>` + initials — founder post-deploy DevTools smoke on `/` still required before treating as closed; **S2 burn-in unchanged**.
