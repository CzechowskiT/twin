...
| Candidate calendar P0 root-cause (#124 week events) | `fix/p0-candidate-calendar-root-cause-2026-06-11` — unblock provider phases from `auth/me`/ops health; `test:candidate-calendar-p0-root-cause` + `test:candidate-calendar-week-events-loading` (`docs/CANDIDATE_CALENDAR_P0_ROOT_CAUSE_FIX_2026-06-11.md`) | ⚠️ **PARTIAL** — CI guards **PASS**; prod week grid **PASS** only when founder confirms events load (no stuck **Ładowanie wydarzeń…**) per `docs/FOUNDER_AUTHENTICATED_PERSONA_SMOKE_RUNBOOK_2026-06-12.md` §5 |
