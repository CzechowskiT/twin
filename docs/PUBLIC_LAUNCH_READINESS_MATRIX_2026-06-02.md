...
| Autonomous apply gate | `autonomous_apply_allowed()` requires verified-readiness statuses; nightly path checks same | ✅ gated |
| Ops trigger-sweep | `POST /auto-apply/trigger-sweep` — `user_has_scrape_ops` allowlist only (`auto_apply_settings.py`) | ✅ ops-only |
| UI safety | `test:dashboard-ux-safety` — no Run now / trigger in settings; readiness gating | ✅ PASS |
| S6 sweep tests | `test_auto_apply_trigger_sweep_admin_gate.py` on checklist | ✅ shipped |
| Nightly beat infra | Celery schedule exists; **not exercised** in this audit | ⚠️ infra LIVE; **policy PAUSED** for launch |
