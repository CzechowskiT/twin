...
| R-007 | `POST /beta/waitlist/{code}/cv` accepts uploads with **no rate limit** and only a referral code as auth            | H   | 🔴     | None (size cap exists, count cap does not)                                                                                    | One-file patch: `@limiter.limit("5/minute")` per IP + `request: Request` param                            | `P1_PUBLIC_ENDPOINT_ABUSE_AUDIT_2026-05-27.md` § R-007                                                 |
| R-008 | `POST /beta/waitlist/{code}/voice` — identical risk profile to R-007                                              | H   | 🔴     | Same                                                                                                                          | Same patch idea, same file                                                                                | `P1_PUBLIC_ENDPOINT_ABUSE_AUDIT_2026-05-27.md`                                                         |
| R-009 | `GET /beta/waitlist/{code}` enables referral-code enumeration                                                     | M   | 🔴     | None                                                                                                                          | `@limiter.limit("60/minute")` per IP                                                                      | Same                                                                                                  |
| R-010 | OAuth callback `GET` routes have no rate limit → provider-quota burn loop                                          | L   | 🔴     | None (cookie state-check exists; not a rate cap)                                                                              | `@limiter.limit("30/minute")` per IP on each `*/callback`                                                  | Same                                                                                                  |
| R-011 | `POST /consent/cookies` accepts unlimited writes per IP                                                            | L   | 🔴     | None                                                                                                                          | `@limiter.limit("30/minute")` per IP                                                                      | Same                                                                                                  |
| R-012 | `POST /recruiter/inbox/{id}/respond` no per-token cap → leaked token flips an inbox                                | M   | 🔴     | None (token-based auth exists; no rate limit)                                                                                  | Per-token Layer-2-style limiter (key = recruiter token, budget 60/min on writes)                          | `P1_PUBLIC_ENDPOINT_ABUSE_AUDIT_2026-05-27.md`                                                         |
...
| R-019 | Data-subject access (export / delete) is partial                                                                   | M   | 🟡     | `/auth/me` returns the profile; bulk export + hard-delete are manual today                                                     | Ship `/api/v1/auth/me/export` and `/api/v1/auth/me/delete-account` (account self-service)                  | `PUBLIC_LAUNCH_GATE_CHECKLIST_2026-05-27.md` gate L6                                                  |
...
- **🔴 Open (8):** R-007, R-008, R-009, R-010, R-011, R-012,
  R-018, R-021, R-022 (some L sev, one H, three M).
- **🟡 Partial (7):** R-004, R-013, R-014, R-017, R-019,
  R-020, R-024.
- **🟢 Closed (10):** R-001, R-002, R-003, R-005, R-006,
  R-015, R-016, R-023, R-025, plus the structural ones
...
1. **R-007 / R-008** — rate-limit beta CV / voice uploads
   (single-file patch + 1 test file). **HIGH severity, low
   cost → ship next.**
