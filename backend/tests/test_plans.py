...
def test_effective_plan_standby_when_active() -> None:
    assert effective_plan_tier(_u(plan_tier="standby", subscription_status="active")) is PlanTier.STANDBY


def test_max_tracked_free_cap() -> None:
    assert max_tracked_applications(PlanTier.FREE) == 25
    assert max_tracked_applications(PlanTier.STANDBY) == 25
    assert max_tracked_applications(PlanTier.STANDARD) is None
