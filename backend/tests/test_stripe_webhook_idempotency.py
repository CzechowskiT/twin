...
def _invoice_payment_failed_payload(event_id: str) -> bytes:
    """`invoice.payment_failed` is unhandled but must still dedup on replay."""
    return json.dumps(
        {
            "id": event_id,
...
def _checkout_session_expired_payload(event_id: str) -> bytes:
    """`checkout.session.expired` is unhandled but must still dedup on replay."""
    return json.dumps(
        {
            "id": event_id,
...
def _checkout_async_payment_failed_payload(event_id: str) -> bytes:
    """`checkout.session.async_payment_failed` is unhandled but must still dedup."""
    return json.dumps(
        {
            "id": event_id,
...
def test_invoice_payment_failed_replay_is_marked_ignored_and_deduped(
    client: tuple[TestClient, sessionmaker],
) -> None:
    """`invoice.payment_failed` replay should return `replayed=true` and stay ignored."""
    http, session_local = client
    event_id = "evt_idempotency_invoice_payment_failed_001"
    payload = _invoice_payment_failed_payload(event_id)
    headers = {"Content-Type": "application/json", "stripe-signature": _sign(payload)}
...
def test_checkout_session_expired_replay_is_marked_ignored_and_deduped(
    client: tuple[TestClient, sessionmaker],
) -> None:
    """`checkout.session.expired` replay should return `replayed=true` and stay ignored."""
    http, session_local = client
    event_id = "evt_idempotency_checkout_expired_001"
    payload = _checkout_session_expired_payload(event_id)
    headers = {"Content-Type": "application/json", "stripe-signature": _sign(payload)}
    ...
    try:
        row = db.query(StripeWebhookEvent).filter_by(event_id=event_id).one()
        assert row.event_type == "checkout.session.expired"
        assert row.handler_status == stripe_events.STATUS_IGNORED
    finally:
        db.close()
