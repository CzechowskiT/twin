"""Recruiter Wave 2 API — Hard LIVE evidence + wave status (no LIVE badge inflation)."""
...
@router.get("/status")
def get_wave2_status(
    db: Session = Depends(get_db),
    _user: User = Depends(get_current_user),
) -> dict:
    return wave2.wave2_status(db)
...
def get_hard_live_evidence(
    db: Session = Depends(get_db),
    _user: User = Depends(get_current_user),
    persona: str | None = Query(default="recruiter", max_length=32),
    wave: str | None = Query(default="2", max_length=16),
) -> dict:
    return wave2.list_hard_live_evidence(db, persona=persona, wave=wave)
...
def mark_hard_live_evidence(
    ...
    try:
        return wave2.mark_evidence_after_smoke(
            db,
